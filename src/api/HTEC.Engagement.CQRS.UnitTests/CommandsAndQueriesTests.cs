...
namespace HTEC.Engagement.CQRS.UnitTests
{
    /// <summary>
    ...
    public class CommandsAndQueriesTests
    {
        ...
        {
            var assembly = typeof(CreatePoints).Assembly;
        ...
        {
            var definitions = typeof(CreatePoints).Assembly.GetImplementationsOf(typeof(ICommand));
        ...
        {
            var definitions = typeof(CreatePoints).Assembly.GetImplementationsOf(typeof(IOperationContext));
        ...
        {
            var definitions = typeof(GetPointsById).Assembly.GetImplementationsOf(typeof(IQueryCriteria));
            foreach (var definition in definitions)
            {
                var operationCode = GetOperationCode(definition.implementation);
                var operationName = GetOperationName(operationCode);
        ...
        {
            var commands = typeof(CreatePoints)
                .Assembly
        ...
        {
            var queries = typeof(GetPointsById)
                .Assembly
                .GetImplementationsOf(typeof(IQueryCriteria))
                .Select(c => c.Item2);

            var handlers = typeof(GetPointsByIdQueryHandler)
                .Assembly
