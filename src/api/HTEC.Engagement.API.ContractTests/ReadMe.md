# HTEC.Engagement.Api.ContractTests

`HTEC.Engagement.Api.ContractTests` is a sample solution for demostrating Consumer Driven contract tests from the provider point of view.
...
If the tests need to run against a different version of the contracts, version within the source code will need to be updated. See `private string CreatePactUri(string consumerName, string providerName)` within the class `PointsApiProviderTests.cs`

#### Command line
Navigate to the contract test project folder:

`$ cd Path/To/Repo/src/tests/Functional/HTEC.Engagement.API.FunctionalTests`
...
This can be seen within `PointsApiProviderTests.cs`

```c#
        [Theory]
        [InlineData("GenericPointsConsumer")]
