...
public class CommandsAndQueriesTests
{
    ...
    {
        var assembly = typeof(CreateWallet).Assembly;
    ...
    {
        var definitions = typeof(CreateWallet).Assembly.GetImplementationsOf(typeof(ICommand));
    ...
    {
        var definitions = typeof(CreateWallet).Assembly.GetImplementationsOf(typeof(IOperationContext));
    ...
    {
        var definitions = typeof(GetWalletById).Assembly.GetImplementationsOf(typeof(IQueryCriteria));
        foreach (var definition in definitions)
        {
            var operationCode = GetOperationCode(definition.implementation);
            var operationName = GetOperationName(operationCode);
    ...
    {
        var commands = typeof(CreateWallet)
            .Assembly
    ...
    {
        var queries = typeof(GetWalletById)
            .Assembly
            .GetImplementationsOf(typeof(IQueryCriteria))
            .Select(c => c.Item2);

        var handlers = typeof(GetWalletByIdQueryHandler)
            .Assembly
