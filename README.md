...
- The `functions` folder contains 3 sub-folders with Azure Functions solutions
  - `func-aeh-listener` is an Azure Event Hub trigger that listens for `WalletCreatedEvent`
  - `func-asb-listener` is an Azure Service Bus subscription (filtered) trigger that listens for `WalletCreatedEvent`
...
% dotnet new stacks-az-func-cosmosdb-worker -n Foo.Bar -do Wallet
...
4. Inside the `Stacks` database create a container called `Wallet`
